...
export default function Verify({ userId }) {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post(`https://pipsville-img-server.cyclic.app/user/verify/${userId}`, photoData, config)
      .then(() => {
        setLoading(false);
      ...
      <Container>
        <RootStyle>
          ...
          <Typography sx={{ px: 4 }} variant="body1">
            {' '}
            verify your account now using valid snapshot of personal identity card
          </Typography>
          ...
          >
            <Zone
              {...getRootProps({
                style,
                className: `drop-zone ${acceptedFiles && acceptedFiles.length ? 'bg-[#AAF27F]' : ''}`,
              })}
            >
              <input {...getInputProps()} />
              {acceptedFiles && acceptedFiles.length ? (
                <>
                  <Typography variant="subtitle2">{acceptedFiles[0].name}</Typography>
                </>
              ) : (
                <>
                  <Typography variant="subtitle2">Click or drop ID here</Typography>
                  <Typography variant="body1">(max size :3mb, *.jpeg and *.png )</Typography>
                </>
              )}
            </Zone>
            <LoadingButton
