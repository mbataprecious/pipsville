...
export const reset = async (req, res) => {
  try {
    ...
    const token = await jwtSign({ user: fetchedUser._id }, config.jwtSecret, { expiresIn: 60 * 60 * 24 });
    let hostname = req.headers.host;
    let verificationLink = `http://${hostname}/reset/${token}`;
    let msg = resetPasswordMailTemplate(fetchedUser.firstName, verificationLink);
...
export const updatePassword = async (req, res) => {
  ...
  try {
    ...
    const newDetails = await User.findByIdAndUpdate(_id, { password: hash }).lean();
